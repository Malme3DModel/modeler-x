...
**実装ファイル**: 
- `components/threejs/ThreeJSViewport.tsx` - メインの実装
- `components/threejs/HoverTooltip.tsx` - ツールチップ表示
**実装内容**:
- フェイス/エッジのハイライト表示
- マウス追従ツールチップでのインデックス表示
- 元のマテリアル保存・復元機能
- パフォーマンス最適化
...
**説明**: 元のMatCapマテリアルの再現
**実装ファイル**: 
- `components/threejs/materials/MatCapMaterial.tsx` - MatCapマテリアル
- `components/threejs/ThreeJSViewport.tsx` - ライティングとフォグ
- `public/textures/dullFrontLitMetal.png` - MatCapテクスチャ
**実装内容**:
- MatCapマテリアルの実装
- 適切なライティング設定（環境光、半球光、平行光源）
- 動的フォグの実装（バウンディングボックスに基づく計算）
- シャドウ表示の設定

### 1.3 トランスフォームハンドル（ギズモ）
